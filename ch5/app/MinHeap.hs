module MinHeap (items, empty, insert, deleteMin, viewMin, weights) where
...
deleteMin (Heap xs)
  ...
  where
    items' = set (element 1) (last xs) (init xs)

viewMin heap@(Heap (_ : y : _)) = Just (value y, weight y, deleteMin heap)
viewMin _ = Nothing
...
percolateDown i items
  ...
  where
    ...
    i' =
      if (weight (fromJust left)) < (weight (fromJust right))
        then l
        else r
    l = 2 * i
    r = 2 * i + 1
